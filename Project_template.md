...
* Реализовал 1 микросервис, представляющий собой сервис управления датчиками.
* Взаимодействие между монолитом и новыми сервисами организовал таким образом, что управление температурными датчиками и снятие их показаний производится через монолит (в коде и настройках фигурирует как legacy*),
  а все новые функции берет на себя новый микросервис.
* В директории warmhouse обновленный docker-compose в котором теперь содержится новый микросервис
* В директории warmhouse обновленный init.sql с расширенной БД
